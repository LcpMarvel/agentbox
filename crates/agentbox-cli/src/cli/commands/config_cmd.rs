...
pub async fn execute(args: Vec<String>) -> anyhow::Result<()> {
    ...
    match args[0].as_str() {
        "set" => {
            ...
            let resp = ipc_call("config.set", serde_json::json!({
                "key": args[1],
                "value": args[2],
            })).await?;
            if resp.result.is_some() {
                println!("{} Config set: {} = {}", "✓".green(), args[1].bold(), args[2]);
        ...
        "get" => {
            ...
            let resp = ipc_call("config.get", serde_json::json!({
                "key": args[1],
            })).await?;
        ...
        "max_concurrent" => {
            ...
            if resp.result.is_some() {
                println!("{} Max concurrent agents set to {}", "✓".green(), args[1].bold());
            } else if let Some(e) = resp.error {
                eprintln!("{} {}", "✗".red(), e.message);
            }
        }
        "alert.webhook" => {
            ...
            let resp = ipc_call("alert.add", serde_json::json!({
                "channel": "webhook",
                "config": {"url": args[1]},
            })).await?;
            if resp.result.is_some() {
        ...
        "alert.list" => {
            ...
            if let Some(result) = resp.result {
                let channels: Vec<serde_json::Value> = serde_json::from_value(result).unwrap_or_default();
                if channels.is_empty() {
                    println!("No alert channels configured.");
                } else {
                    println!("{:<5} {:<12} {:<8} {}", "ID", "Channel", "Enabled", "Config");
                    println!("{}", "-".repeat(60));
                    for ch in channels {
                        println!("{:<5} {:<12} {:<8} {}",
                            ch.get("id").and_then(|v| v.as_i64()).unwrap_or(0),
                            ch.get("channel").and_then(|v| v.as_str()).unwrap_or(""),
                            if ch.get("enabled").and_then(|v| v.as_bool()).unwrap_or(false) { "✅" } else { "❌" },
